...
class RenderStrategy(ABC):
    ...
    def prepare_render_data(self, size: int) -> RenderAttrs:
        ...
        def _v_match(a: float, b: float, eps: float = 0.01) -> bool:
            return abs(float(a) - float(b)) < eps

        # Vertical angle span sets which rows of the square image receive the wall render.
        # Many Type C floods use V ∈ [-90°, 90°]; older code only handled 0–180 / 0–90 / 90–180.
        if _v_match(start_v_angle, 0) and _v_match(end_v_angle, 180):  # V ∈ [0, 180]
            # the distribution lies in both hemispheres
            y_start, y_end = 0, size
        elif _v_match(start_v_angle, 0) and _v_match(end_v_angle, 90):  # V ∈ [0, 90]
            # distribution lies completely in the bottom hemisphere
            y_start, y_end = size // 2, size
        elif _v_match(start_v_angle, 90) and _v_match(end_v_angle, 180):  # V ∈ [90, 180]
            # the distribution lies completely in the top hemisphere
            y_start, y_end = 0, size // 2
        elif _v_match(start_v_angle, -90) and _v_match(end_v_angle, 90):  # V ∈ [-90, 90]
            # full vertical range (common for Type C floods / wide grids)
            y_start, y_end = 0, size
        else:
            # Unusual or float-edge grids: still render something valid
            y_start, y_end = 0, size
